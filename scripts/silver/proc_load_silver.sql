...
BEGIN
	...
	BEGIN TRY
		...
		*,
		...
		END AS sls_due_dt,
		CASE WHEN sls_sales <= 0 OR sls_sales IS NULL OR sls_sales != sls_quantity*sls_price THEN ABS(sls_quantity * sls_price)
			 ELSE sls_sales
		END AS sls_sales,
		sls_quantity,
		...
		CASE WHEN UPPER(TRIM(cntry)) IN ('DE', 'GERMANY') THEN 'Germany' 
			 ...
			 WHEN UPPER(TRIM(cntry)) IS NULL OR LEN(UPPER(TRIM(cntry))) = 0 THEN 'n/a'
			 ELSE cntry
		END AS cntry
